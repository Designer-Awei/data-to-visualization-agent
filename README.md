...
8. **绘图数据计算agent（仅绘图链路，按需）**
   - 若需二次计算，LLM生成pandas代码，后端安全执行，得到新数据。
   - 日志打印：输入数据、生成代码、执行结果。

9. **智能绘图agent**
   - 输入：用户问题 + 结构化绘图数据（DataFrame/result/合并数据）。
   - 负责理解绘图意图，确定图表类型、x/y轴、分组等，输出详细绘图需求或伪代码/结构化描述。
   - 日志打印：输入数据、绘图意图、输出描述。

10. **智能代码生成agent**
   - LLM根据绘图需求，生成最终可执行的Python（plotly）代码，保证格式正确。
   - 日志打印：输入需求、生成代码。
...
### 设计目标
- 当智能绘图agent生成的代码在后端执行失败时，自动调用"代码修复agent"进行修正，提升系统健壮性和泛化能力。

### 输入
- 用户在前端输入的自然语言问题
- 智能绘图agent生成的（执行失败的）Python代码
- 后端捕获的详细报错信息（如Python异常堆栈、错误类型、错误行号等）

### 输出
- "代码修复agent"根据报错信息先进行错误推理，然后自动修正后的新Python代码（优先修正常见低级错误，如变量未定义、range类型、x/y长度不符等，也可泛化为更复杂的修正）

### 作用
- 自动修正LLM生成的代码bug，减少人工干预，提升端到端自动化能力
- 保证复杂/新场景下的泛化能力，最大化利用LLM的创造力，同时兜底常见低级错误

### 典型流程
1. 智能绘图agent生成代码，后端执行失败
2. 捕获失败代码和报错信息，传递给代码修复agent
3. 代码修复agent输出修正后的新代码，后端再次尝试执行
4. 若多次修正仍失败，则走最终兜底逻辑

### 代码修复agent已实现
- 当智能代码生成agent生成的Python代码执行失败时，自动调用代码修复agent，最多修复3次。
- **现在智能绘图主流程和绘图数据计算agent均支持自动修复，失败时最多自动修复3次，极大提升健壮性。**
- 代码修复agent已封装为通用函数，主流程和数据计算agent均复用，输入输出一致，便于维护和扩展。
- 智能绘图agent、智能代码生成agent、代码修复agent均已在后端加上终端日志输出，便于调试和追踪。
