...
- 步骤：
  1. LLM 根据用户问题和数据，生成 Python 代码（如均值、分组、聚合、差值、总分等）。
  2. 计算 agent 在 Node.js 后端安全执行代码，返回真实计算结果。
  3. 若代码执行失败，自动重试或返回 LLM 口算结果并提示风险。
  4. 计算结果支持多样化结构：
     - 若为多字段（如多科目平均分），以表格结构返回。
     - 若为单一数值（如差值、总分），以分析文本返回，analysis 字段需引用所有中间结果和最终结果。
     - prompt 会根据计算结果类型智能切换输出结构，确保所有中间结果和最终结果都能被引用和展示。

### 4. 二次组织语言（LLM 二次解释 agent）
- 作用：让 LLM 根据"用户问题+代码+真实计算结果+涉及字段"用自然语言详细解释思路、过程和结论，提升用户体验。
- 步骤：
  1. 将用户问题、代码、涉及字段、真实计算结果拼接为新 prompt。
  2. LLM 生成自然语言答案，要求必须直接引用真实计算结果和所有中间结果，不得自行推算。
  3. 输出结构根据计算结果类型自动切换：
     - 多字段时输出 table+analysis
     - 单一数值时仅输出 analysis，analysis 中需包含所有中间结果
  4. 前端展示 LLM 最终自然语言回复。
