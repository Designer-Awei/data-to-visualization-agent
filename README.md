...
5. **数据处理逻辑agent（仅绘图链路）**
   - 判断绘图数据是否需要二次计算（如总分、均值、分组统计等）。
   - 输出数据状态：`origin`（直接绘图）、`todo`（需二次计算）、`both`（两者都用）。

6. **绘图数据计算agent（仅绘图链路，按需）**
   - 若需二次计算，LLM生成pandas代码，后端安全执行，得到新数据。
   - 日志打印：输入数据、生成代码、执行结果。

7. **智能代码生成agent**
   - LLM根据拼接的prompt，**强制只输出Python代码块**，且代码最后必须有`result = {...}`和`print(json.dumps(result, ensure_ascii=False))`。
   - 严格禁止多余print、分析文本、调试输出。
   - 后端严格校验代码块格式，若不合格则自动重试。
   - 日志打印：输入数据、生成代码。

8. **后端安全执行Python代码**
   - Node.js后端用`child_process.spawn`安全执行LLM生成的代码，得到真实结构化结果（数值/figure JSON）。
   - 兼容多行输出，只取最后一行合法JSON。
   - 日志打印：执行结果。
...
10. **前端渲染**
    ...
    - 图表用react-plotly.js渲染figure JSON，支持下载PNG/SVG/HTML。
    - 遇到异常时，前端问答区始终显示"【LLM服务异常】...兜底，绝不空白。

> 该流程确保了“意图识别-字段提取-数据检索-数据处理-强制代码-安全执行-结构化分析-前端渲染”全链路智能协作，极大提升了问答与绘图的准确性、健壮性和用户体验。
